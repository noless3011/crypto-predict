...
def add_technical_indicators(df):
    ...
    print(
        f"  ✓ Added {len([col for col in df.columns if col not in ['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'Date_Str', 'Date_Only', 'Gold']])} indicators"
    )
...
def create_transformer_model(
    input_seq, output_seq, n_features, d_model=64, num_heads=4, ff_dim=256, blocks=2
):
    """
    ...
    for _ in range(blocks):
        ...
        ff = Dropout(0.1)(ff)  # Reduced from 0.5
        ff = Dense(d_model)(ff)
        x = LayerNormalization(epsilon=1e-6)(x + ff)

    # Output: Use all timesteps, not just last
    x = Flatten()(x)
    x = Dense(128, activation="relu")(x)
    x = Dropout(0.2)(x)
    outputs = Dense(output_seq)(x)

    model = Model(inputs, outputs)
    model.compile(optimizer=Adam(learning_rate=0.0001), loss="mse")  # Lower LR
    return model
...
def train_and_forecast(model, train_X, train_y, test_X, test_y, epochs, verbose):
    ...
    model.fit(
        ...
        batch_size=32,
        verbose=verbose,
        callbacks=[progress_callback, history],
    )
...
def eval_result(result, target, n_steps_out, is_log_return=False):
    ...
    for i in range(n_steps_out):
        ...
        mape = (
            np.mean(np.abs((target[:, i] - result[:, i]) / (target[:, i] + 1e-9))) * 100
        )
...
def visualize_best_model(model_name, dates, y_true, y_pred, n_steps_out):
    ...
    ax1.set_title(
        f"{model_name}: Full Price Prediction (Daily Data)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax2.plot(
        plot_dates[:zoom_samples],
        y_true_1step[:zoom_samples],
        label="Actual",
        color="black",
        linewidth=2,
        marker="o",
    ...
    ax2.set_title(
        f"Zoom View - First {zoom_samples} Days", fontsize=12, fontweight="bold"
    )
    ...
    ax3.set_title(
        f"Zoom View - Last {zoom_samples} Days", fontsize=12, fontweight="bold"
    )
    ...
    ax4.set_title(
        f"{model_name}: Actual vs Predicted Price", fontsize=12, fontweight="bold"
    )
...
def visualize_log_returns(model_name, test_dates, y_true_log, y_pred_log, n_steps_out):
    ...
    ax1.set_title(
        f"{model_name} - Full Log Return Predictions (1-step ahead)",
        fontsize=14,
        fontweight="bold",
    )
    ax1.set_xlabel("Date", fontweight="bold")
    ...
    plt.savefig(
        f"log_returns_{model_name.replace(' ', '_')}.png", dpi=300, bbox_inches="tight"
    )
    print(
        f"✓ Saved log return visualization to 'log_returns_{model_name.replace(' ', '_')}.png'"
    )
    plt.show()
...
def plot_training_losses(all_losses, epochs):
    ...
    fig, axes = plt.subplots(
        (n_models + 1) // 2, 2, figsize=(16, 5 * ((n_models + 1) // 2))
    )
    ...
    for idx, (model_name, losses) in enumerate(all_losses.items()):
        ...
        ax.plot(
            epochs_range, mean_loss, color="darkblue", linewidth=2.5, label="Mean Loss"
        )
        ax.fill_between(
            epochs_range,
            mean_loss - std_loss,
        ...
        # Styling
        ax.set_title(
            f"{model_name} - Training Loss ({len(losses)} rounds)",
            fontsize=12,
            fontweight="bold",
        )
        ax.set_xlabel("Epoch")
...
def plot_final_loss_comparison(all_losses):
    ...
    plt.title(
        "Final Training Loss Comparison (Mean ± Std Dev)",
        fontsize=14,
        fontweight="bold",
    )
...
try:
    data, data_len, dates, original_close_prices = read_data(
        file_path, dim_type, use_percentage, use_indicator
    )
...
model_list = [
    ...
    "MLP",
    "Transformer",
]  # Add other models back here: 'LSTM', 'CNN', etc.
verbose = 1
...
for model_name in model_list:
    ...
    for r in range(rounds):
        ...
        # Train and predict
        train_pred, test_pred, loss_history = train_and_forecast(
            model, train_X, train_y, test_X, test_y, current_epochs, verbose
        )
        ...
        for i in range(n_steps_out):
            train_pred_inv[:, i] = scaler_close.inverse_transform(
                train_pred[:, i].reshape(-1, 1)
            ).ravel()
            test_pred_inv[:, i] = scaler_close.inverse_transform(
                test_pred[:, i].reshape(-1, 1)
            ).ravel()
            train_y_inv[:, i] = scaler_close.inverse_transform(
                train_y[:, i].reshape(-1, 1)
            ).ravel()
            test_y_inv[:, i] = scaler_close.inverse_transform(
                test_y[:, i].reshape(-1, 1)
            ).ravel()

        # 1. Evaluate Log Returns
        log_metrics = eval_result(
            test_pred_inv, test_y_inv, n_steps_out, is_log_return=True
        )
        ...
        for i in range(len(train_pred_inv)):
            ...
            train_pred_price[i, :] = inverse_log_returns(
                train_pred_inv[i, :], initial_price
            )
        ...
        for i in range(len(test_pred_inv)):
            ...
            test_pred_price[i, :] = inverse_log_returns(
                test_pred_inv[i, :], initial_price
            )
            test_y_price[i, :] = inverse_log_returns(test_y_inv[i, :], initial_price)

        # 3. Evaluate Prices
        price_metrics = eval_result(
            test_pred_price, test_y_price, n_steps_out, is_log_return=False
        )
        ...
        print(
            f"   [Log Returns] RMSE: {log_metrics['rmse_avg']:.5f}  | MAPE: {log_metrics['mape_avg']:.2f}%"
        )
        print(
            f"   [Price]       RMSE: {price_metrics['rmse_avg']:.2f}  | MAPE: {price_metrics['mape_avg']:.2f}%"
        )
        ...
        if test_rmse < best_rmse:
            ...
            best_test_dates = dates[
                test_dates_start : test_dates_start + len(test_y_inv)
            ]
    ...
    results.append(
        {"Model": model_name, "Avg Test RMSE": avg_rmse, "Avg Test MAPE": avg_mape}
    )
